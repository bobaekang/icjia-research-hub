...
function filterAuthorInfo(articleInfo, authorInfo) {
  ...
  filteredAuthorInfo.push(
    ...[
      ...
      {
        title: 'Sex Offenses and Sex Offender Registration Task Force',
        description: ''
      }
    ]
  )
...
async function editArticleInfo(info) {
  const temp = info.map(el => {
    ...
    el.authorNames = el.authors.map(el => {
    ...
    if (el.hasOwnProperty('pdf_uploads')) {
      el.pdf_uploads.forEach(upload => {
        if (
          upload.hasOwnProperty('reportType') &&
          upload.reportType === 'Presentation'
        ) {
...
async function addMarkdown(arr) {
  ...
  const promises = arr.map(el => {
    return JSDOM.fromURL(`http://www.icjia.state.il.us/articles/${el.filename}`)
  })
  ...
  results.forEach((dom, i) => {
    try {
      const html = dom.window.document.querySelector('.article-content')
        .outerHTML
